...
TEX = {
    ...
    "plu": r"PA=LU",
    "lu_solve": r"Ly=Pb,\qquad Ux=y",
    "lu_det": r"\det(A)=\det(P)\prod_i u_{ii}",
    "chol": r"A=R^{\mathsf{T}}R",
    "chol_solve": r"R^{\mathsf{T}}y=b,\qquad Rx=y",
    "chol_props": r"\det(A)=\prod_i r_{ii}^2>0,\qquad\kappa_2(A)=\kappa_2(R)^2",
    ...
    "qr_ls": r"Rx=Y^{\mathsf{T}}b,\qquad b-YY^{\mathsf{T}}b\perp\operatorname{col}(A)",
    "svd_sums": r"A=\sum_{j=1}^{r}\sigma_ju_jv_j^{\mathsf{T}},\qquad Av_i=\sigma_iu_i,\qquad u_i^{\mathsf{T}}A=\sigma_iv_i^{\mathsf{T}}",
    "svd_spectral": r"\sigma_i^2=\lambda_i(A^{\mathsf{T}}A),\qquad\Vert A\Vert_2=\sigma_1",
    "svd_pinv": r"A^+=V[\Sigma^+\ \ 0]U^{\mathsf{T}},\qquad x_{\min}=A^+b",
    ...
    "bs_pde_grid": r"V_t(S_j,t_n)+rS_jV_S(S_j,t_n)+\frac{1}{2}\sigma^2S_j^2V_{SS}(S_j,t_n)=rV_j^n",
...
def front_columns(S):
    ...
    c2 += topic("3", "MATRIX STRUCTURE & FACTORISATIONS", [
        ...
        ("sub", "LU (general square systems)"),
        ("body", f"With partial pivoting {B('PA=LU')}, L unit lower and U upper; |ℓᵢⱼ|≤1. Solve Ax=b by {equation('lu_solve')}; {equation('lu_det')}. Factor cost ≈2n³/3, then ≈2n² per RHS; reuse factors for many b. {R('Require nonsingular A for a unique solve; do not form A⁻¹.')}"),
        ("sub", "Cholesky (symmetric PD systems)"),
        ("body", f"{B('A=RᵀR')}, R upper. {R('Require real symmetric positive-definite A.')}; rᵢᵢ>0 makes R unique (otherwise DR, D=diag(±1), also factors A). Solve Ax=b by {equation('chol_solve')}. {equation('chol_props')}. Factor cost n³/3, then 2n² per RHS; no row pivoting."),
        ("sub", "QR (least squares / rank)"),
        ("body", f"For A∈ℝᵐˣⁿ, m≥n, rank(A)=n: {equation('qr_economy')}; R nonsingular upper. For min<sub>x</sub>‖Ax−b‖₂, {equation('qr_ls')}. {R('Avoid normal equations when conditioning matters.')} Cost ≈2mn²−(2/3)n³. If rank deficient: {B('AP=Q[R;0]')}, column pivoting, |r₁₁|≥⋯≥|rₙₙ|; small diagonal entries indicate numerical rank deficiency."),
        ("sub", "SVD (rank / pseudoinverse)"),
        ("body", f"A∈ℝᵐˣⁿ, m≥n: {B('A=U[Σ;0]Vᵀ')}; U,V orthogonal, Σ=diag(σ₁≥⋯≥σₙ≥0). rank(A)=number of positive σᵢ; {equation('svd_sums')}; {equation('svd_spectral')}. Full rank: κ₂(A)=σ₁/σₙ. {equation('svd_pinv')} gives least-squares x for full column rank and minimum-norm least-squares x when rank deficient; Σ⁺ uses reciprocals of retained positive σᵢ. Numerical rank tolerance ≈max(m,n)εσ₁."),
    ], S)
...
def back_columns(S):
    ...
    c4 += topic("11", "FDMs FOR BLACK–SCHOLES", [
        ("sub", "Grid, notation and differences"),
        ("body", f"Truncate S∈[0,S<sub>max</sub>], {R('S<sub>max</sub>>K and sufficiently large')}. {equation('fd_grid')}. {equation('fd_value')}.<br/>{equation('bs_pde_grid')}.<br/>{equation('fd_time_backward')}.<br/>{equation('fd_space')}; each O((ΔS)²). Boundary/terminal values from Topic 10."),
